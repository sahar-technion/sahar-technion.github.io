...
---
## Intro
Like math has numbers and symbols, each language has it's own symbols, syntax and semantics which are used to explain and convey information and ideas. The way this language is contructed is very important since it affects what you can say, how difficult it is to say it, what the main focus is and so on.
...
Lastly, we would like every word you can write in the language to have a meaning aka a corresponding valid molecule, but unfortunatelly that is not the case with SMILES. Still I choose to talk about this language and not SELFIES its postdecessor since SMILES is the most commonly used language nowadays in the cheminformatics community - libraries and models are built around it and therefore it's much easier to base yourself on it, although it has this major caviat.
...
### Rings
Rings are encoded by numbering the two atoms that are connected across the ring closure. For example, `C1CCCCC1` closes a six-membered saturated ring, and `c1ccccc1` closes an aromatic six-membered ring.
